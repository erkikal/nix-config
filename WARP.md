...
- **Architecture**: Host-centric modular configuration structure.
  - Targets hosts directly (`mkNixosConfig host`) via `hosts = [ "zaneyos-next" "default" "zaneyos-24-vm" "zaneyos-oem" ]`.
...
- **Repo location**: `nh` and `zcli` assume `~/zaneyos`; if located elsewhere, adjust `modules/core/nh.nix`.
